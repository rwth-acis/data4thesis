function Data4Thesis(url) {
  this.data = {
    basic: {
      ...
      webxr: (navigator.xr && navigator.xr.isSessionSupported('immersive-vr')),
...
padding-top: 1rem;
  ...
  for (btn of starsBtns) {
    btn.onclick = (event) => {
      const el = event.currentTarget;
      this.rating = el.dataset.stars;
      stars.classList.remove('hover');
      for (b of starsBtns) {
